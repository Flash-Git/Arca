...
contract Trade {
    ...
    function executeFunctionCalls(address _add) private {
        for(uint8 i = 0; i < tradeWindows[_add].funcOffers.length; i++){
            address contractAd = tradeWindows[_add].funcOffers[i].contractAd;
            bytes memory encodedFunc = tradeWindows[_add].funcOffers[i].encodedFunc;
            bool status;
            bytes memory result;
